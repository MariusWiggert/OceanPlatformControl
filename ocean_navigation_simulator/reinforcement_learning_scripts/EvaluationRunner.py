...
class EvaluationRunner:
    def __init__(
        ...
        if verbose > 0:
            print(f'EvaluationRunner: running {len(problems)} missions with {controller_class.gpus} gpus')

        ray_results = ray.get([self.evaluation_run.options(num_gpus=controller_class.gpus).remote(problem, scenario_name, controller_class, verbose) for problem in problems])
    ...
    def evaluation_run(problem, scenario_name, controller_class, verbose):
        ...
        # Step 1: Create Arena
        start = time.time()
        arena = ArenaFactory.create(scenario_name=scenario_name, problem=problem, verbose=verbose-1)
        ...
        start = time.time()
        controller = controller_class(problem=problem, platform_dict=arena.platform.platform_dict, verbose=verbose-1)
        if verbose > 0:
