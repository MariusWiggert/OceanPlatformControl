...
class GenerationRunner:
    def __init__(self, name, scenario_name, runs, num_batches_per_run, batch_size):
        ...
        results_df.to_csv(f'{results_folder}all_problems.csv')
    ...
    def generation_run(scenario_name, seed, num_batches_per_run, batch_size, results_folder):
        try:
            ...
            for batch in range(num_batches_per_run):
                ...
                batch_results = [problem.to_dict() | run_info | {
                    'batch': batch,
                    'process_batch_time': f'{batch_time:.2f}s',
                    'process_ram': f'{psutil.Process().memory_info().rss / 1e6:.1f}MB',
                    'process_gpu_total': f'{gpu_info.total / 1e6:.1f}MB',
                    'process_gpu_used': f'{gpu_info.free / 1e6:.1f}MB',
                    'process_gpu_free': f'{gpu_info.used / 1e6:.1f}MB',
            ...
            print(f'GenerationRunner: Seed {seed} finished ({time.time()-run_start_time:.1f}s)')
