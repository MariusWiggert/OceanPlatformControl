...
# Two types of data sources: analytical and xarray based ones -> need different default functions.


class XarrayDataSource(abc.ABC):
    ...
    def get_data_over_area(self, x_interval: List[float], y_interval: List[float],
        ...
        subset = self.DataArray.sel(
            time=slice(t_interval[0].replace(tzinfo=None) - datetime.timedelta(
                seconds=self.source_config_dict['subset_time_buffer_in_s']),
                       t_interval[1].replace(tzinfo=None) + datetime.timedelta(
                           seconds=self.source_config_dict['subset_time_buffer_in_s'])),
        ...
        array_subsetting_sanity_check(subset, x_interval, y_interval, t_interval)
    ...
    def check_for_casadi_dynamics_update(self, x_t: List[float]) -> None:
        """Function to check if our cached casadi dynamics need an update because x_t is outside of the area.
            Args:
              x_t: state to check if we have a working casadi function [x, y, battery, posix_time, seaweed mass]
            """
        out_x_range = not (self.casadi_grid_dict['x_range'][0] < x_t[0] < self.casadi_grid_dict['x_range'][1])
        out_y_range = not (self.casadi_grid_dict['y_range'][0] < x_t[0] < self.casadi_grid_dict['y_range'][1])
        out_t_range = not (self.casadi_grid_dict['t_range'][0] < x_t[0] < self.casadi_grid_dict['t_range'][1])
...
class AnalyticalDataSource(abc.ABC):
...
### Functions used by both of them
