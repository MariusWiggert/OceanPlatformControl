...
from ocean_navigation_simulator.env.data_sources.DataSources import AnalyticalDataSource
...
class AnalyticalOceanCurrentSource(AnalyticalDataSource):
    ...
    def create_xarray(self, grid_dict: dict, data_tuple: Tuple) -> xr:
        """Function to create an xarray from the data tuple and grid dict
            ...
            Returns:
              xr     an xarray.Dataset with water_u and water_v
            """
        return xr.Dataset(
    ...
    def map_analytical_function_over_area(self, states, grid_dict) -> Tuple:
        """Function to map the analytical function over an area with the spatial states and grid_dict times.
            Args:
              ...
              grid_dict: containing ranges and grids of x, y, t dimension
            Returns:
              data_tuple     of numpy arrays (u_data, v_data) ordered as 3D matrixes [T, Y, X]
            """
...
class PeriodicDoubleGyre(AnalyticalOceanCurrentSource):
...
class FixedCurrentHighwayField(AnalyticalOceanCurrentSource):
