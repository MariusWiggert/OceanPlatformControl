...
class PFTTree:
	...
	belief_id_to_values: list # id -> Value(b)
...
class PFTDPWPlanner:
	...
	def _insert_belief_node(self, new_belief: ParticleBelief) -> int:
		...
		self.tree.belief_id_to_values.append(0)
	...
	def mcts_simulate(self, belief_id: int, current_depth: int) -> float:
		...
		if current_depth == 0:
			...
			# return self._estimate_rollout_value(belief)
			return 0.0
		...
		if new_node:
			# set the V(b) of the new belief node to the rollout value
			self.tree.belief_id_to_values[next_belief_id] = self._estimate_rollout_value(next_belief_id)
			q_value = reward + self.specific_settings["discount"] * self.tree.belief_id_to_values[next_belief_id]

			# Update the Q(b,a) of the current belief node as the running average (including initial rollout value)
			...
			# Trigger the simulate call in the tree below
			q_value_sample_one_level_down = self.mcts_simulate(next_belief_id, current_depth - 1)
			# update the V(b) of the next_belief_id node via max Q(b,a) over all a (including initial rollout)
			# for next_belief_action_id_children in self.tree.belief_id_to_action_id[next_belief_id]:
			# 	self.tree.belief_id_to_values[next_belief_id] = max(
			# 		self.tree.belief_id_to_values[next_belief_id], # current V(b)
			# 		self.tree.action_id_to_q_values[next_belief_action_id_children])#  Q(b, a_child)
			# probably can implement this faster than a for loop with if > or getting all and then max...

			# without initial rollout
			if len(self.tree.belief_id_to_action_id[next_belief_id]) > 1:
				all_q_values_below_next_belief_id = []
				for next_belief_action_id_children in self.tree.belief_id_to_action_id[next_belief_id]:
					all_q_values_below_next_belief_id.append(self.tree.action_id_to_q_values[next_belief_action_id_children])
				# update it via max
				self.tree.belief_id_to_values[next_belief_id] = max(all_q_values_below_next_belief_id)
			else:
				self.tree.belief_id_to_values[next_belief_id] = self._estimate_rollout_value(next_belief_id)

			# Update the Q(b,a) of the current belief node as the mean of rewards + discounted V(b_child)
			q_value_sum = 0
			for (child_belief_id, reward) in self.tree.action_id_to_belief_id_reward[action_id]:
				q_value_sum += reward + self.specific_settings["discount"] * self.tree.belief_id_to_values[child_belief_id]

			# now update the q_value via the mean
			q_value = q_value_sum / len(self.tree.action_id_to_belief_id_reward[action_id])
			self.tree.action_id_to_q_values[action_id] = q_value
