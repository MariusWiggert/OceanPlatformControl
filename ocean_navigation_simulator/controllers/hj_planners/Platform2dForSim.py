...
class Platform2dForSim(dynamics.Dynamics):
    ...
    def get_opt_ctrl_from_values(
        ...
        n_temporal_buffer_idx=6,
        n_spatial_buffer_idx=10,
    ):
        ...
        val_at_t = interp1d(subsetted_reach_times, subsetted_values, axis=0, kind="linear")(
            time
        ).squeeze()

        # Step 2: get_opt_ctrl_from_values_jit
        u_opt, d_opt = self.get_opt_ctrl_from_val_at_t_jit(
            state=x, time=time, grid=subsetted_grid, val_at_t=val_at_t, upwind_scheme=upwind_scheme
        )
        return u_opt, d_opt
    ...
    def create_subsets(
        time, times, all_values, grid, state, n_temporal_buffer_idx, n_spatial_buffer_idx
    ):
        ...
        subsetted_states = dynamic_slice(
            grid.states,
            (x_idx_start, y_idx_start, 0),
        ...
        subsetted_values = dynamic_slice(
            all_values,
            (t_idx_start, x_idx_start, y_idx_start),
        ...
        new_coords = (
            *(
                dynamic_slice_in_dim(cord_vec, idx_start, 2 * n_spatial_buffer_idx)
                for cord_vec, idx_start in zip(grid.coordinate_vectors, [x_idx_start, y_idx_start])
            ),
        )
        ...
        subsetted_grid = Grid(
            subsetted_states, subsetted_domain, new_coords, grid.spacings, grid.boundary_conditions
        )
        return subsetted_grid, subsetted_reach_times, subsetted_values

    @functools.partial(jax.jit, static_argnames=("self", "upwind_scheme"))
    def get_opt_ctrl_from_val_at_t_jit(
        self, state, time, grid, val_at_t, upwind_scheme=upwind_first.WENO3
    ):
        ...
        grad_at_x_cur = grid.interpolate(
            values=grid.grad_values(values=val_at_t, upwind_scheme=upwind_scheme),
            state=state.flatten(),
        )

        # Step 2: get u_opt and d_opt
        u_opt, d_opt = self.optimal_control_and_disturbance(
            state=state.flatten(), time=time, grad_value=grad_at_x_cur
        )
