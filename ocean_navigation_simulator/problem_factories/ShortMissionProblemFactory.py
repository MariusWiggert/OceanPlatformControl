...
class ShortMissionProblemFactory(ProblemFactory):
    def __init__(
        ...
        self.random = np.random.default_rng(
            self.config["seed"] if self.config["seed"] is not None else 2022
        )

        self.target_x_start = (
            self.config["x_range"][0].deg + self.config["target_distance_from_frame"]
        )
        self.target_x_end = (
            self.config["x_range"][1].deg - self.config["target_distance_from_frame"]
        )
        self.target_y_start = (
            self.config["y_range"][0].deg + self.config["target_distance_from_frame"]
        )
        self.target_y_end = (
            self.config["y_range"][1].deg - self.config["target_distance_from_frame"]
        )
    ...
    def generate_batch(self, batch_size: int) -> [NavigationProblem]:
        ...
        if self.verbose > 0:
            print(
                f"ShortMissionProblemFactory: Batch of {batch_size} created ({time.time()-problem_start_time:.1f}s)"
            )
    ...
    def generate_target(self) -> Union[PlatformState, bool]:
        ...
        target = PlatformState(
            lon=units.Distance(
                deg=self.target_x_start
                + (self.target_x_end - self.target_x_start) * self.random.random()
            ),
            lat=units.Distance(
                deg=self.target_y_start
                + (self.target_y_end - self.target_y_start) * self.random.random()
            ),
            date_time=datetime.datetime.fromtimestamp(
                self.random.integers(self.target_t_start, self.target_t_end).item(),
        ...
        self.hindcast_planner = HJReach2DPlanner(
            ...
            | (
                ...
                if "deg_around_xt_xT_box" not in self.config["hj_planner"]
                or not self.config["hj_planner"]["deg_around_xt_xT_box"]
                else {}
            ),
            verbose=self.verbose - 1,
        )
        ...
        self.hindcast_planner.replan_if_necessary(
            ArenaObservation(
                ...
                true_current_at_state=self.arena.ocean_field.get_ground_truth(
                    target.to_spatio_temporal_point()
                ),
                forecast_data_source=self.arena.ocean_field.hindcast_data_source,
            )
        )
        # Update target time to maximum of planner
        target.date_time = datetime.datetime.fromtimestamp(
            int(self.hindcast_planner.current_data_t_0 + self.hindcast_planner.reach_times[-1]),
            tz=datetime.timezone.utc,
        )

        if self.verbose > 1:
            print(
                f"ShortMissionProblemFactory: Target created ({time.time()-start:.1f}s) {target.to_spatio_temporal_point()}"
            )
    ...
    def generate_starts(self, amount, silent=False) -> Union[List[SpatioTemporalPoint], bool]:
        ...
        points = self.hindcast_planner.sample_from_reachable_coordinates(
            ...
            frame_interval=[
                [
                    self.config["x_range"][0].deg + self.config["start_distance_from_frame"],
                    self.config["x_range"][1].deg - self.config["start_distance_from_frame"],
                ],
                [
                    self.config["y_range"][0].deg + self.config["start_distance_from_frame"],
                    self.config["y_range"][1].deg - self.config["start_distance_from_frame"],
                ],
            ],
        ...
        if self.verbose > 0 and not silent:
            if len(points) < amount:
                print(
                    f"ShortMissionProblemFactory: There are only {len(points)} valid reachable points regarding mission time and minimum distance available but {amount} requested."
                )
            elif self.verbose > 1:
                print(
                    f"ShortMissionProblemFactory: {len(points)} starts created ({time.time()-start_time:.1f}s)"
                )
    ...
    def plot_batch(self, batch_size: int, filename: str, random_sample_points: Optional[int] = 10):
        ...
        if self.verbose > 0:
            print(
                f"ShortMissionProblemFactory: Batch of {batch_size} plotted ({time.time()-plot_start_time:.1f}s)"
            )
