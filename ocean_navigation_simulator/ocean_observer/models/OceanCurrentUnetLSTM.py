...
class OceanCurrentUnetLSTM(nn.Module):
    ...
    def __init__(self, ch_sz: list[int], device: str = 'cpu', init_weights_value: float = 0.01, activation="relu",
        ...
        self.bloc_right_3 = nn.Sequential(self.__get_upsizing_bloc(ch_sz[3], ch_sz[3], output_padding=1),
                                          self.__get_same_dims_bloc(ch_sz[3], ch_sz[3]),
                                          self.__get_same_dims_bloc(ch_sz[3], ch_sz[3]))
        self.bloc_right_2 = nn.Sequential(self.__get_upsizing_bloc(ch_sz[3], ch_sz[2], output_padding=1),
        ...
        self.lstm = OceanCurrentRNN(input_size, lstm_hidden, [2, 24, 24], "lstm", num_layers=2,
                                    bidirectional=bidirectional, dropout=dropout, index_time_dimension=self.dim_time,
                                    device=device)
    ...
    def forward(self, x):
        ...
        x_pre_lstm = torch.swapaxes(x_cnn.reshape(batch_size, seq_len, currents_dim, lat, lon), self.dim_time,
                                    self.dim_currents)
