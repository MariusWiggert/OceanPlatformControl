...
def vector_correlation(
    ...
    if per_hour:
        ...
        return {}
    correlations = dict()
    correlations["vector_correlation_improved"] = calc_vector_corr_over_time(
        improved_predictions, ground_truth, sigma_diag=0, remove_nans=True
    ).mean()
    correlations["vector_correlation_initial"] = calc_vector_corr_over_time(
        initial_predictions, ground_truth, sigma_diag=0, remove_nans=True
    ).mean()
    correlations["vector_correlation_ratio"] = correlations["vector_correlation_improved"] / (
        correlations["vector_correlation_initial"] + sigma_ratio
    )
    return correlations
