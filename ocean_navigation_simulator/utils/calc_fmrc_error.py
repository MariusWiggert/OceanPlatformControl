...
def calc_fmrc_errors(problem, T_horizon, deg_around_x0_xT_box, hours_to_abs_time=3600):
    ...
    dict_of_relevant_fmrcs = list(
        filter(lambda dic: dic['t_range'][0] < problem.t_0 + timedelta(hours=T_horizon),
    ...
    for fmrc_dict in dict_of_relevant_fmrcs:
        ...
        RMSE_across_fmrc.append(calc_speed_RMSE(u_data_forecast, v_data_forecast, u_data_hindcast, v_data_hindcast))
        angle_diff_across_fmrc.append(calc_abs_angle_difference(u_data_forecast, v_data_forecast, u_data_hindcast, v_data_hindcast))
        vec_corr_across_fmrc.append(calc_vector_corr_over_time(u_data_forecast, v_data_forecast, u_data_hindcast, v_data_hindcast))
...
def calc_speed_RMSE(u_data_forecast, v_data_forecast, u_data_hindcast, v_data_hindcast):
    ...
    RMSE_speed = np.sqrt((u_data_forecast-u_data_hindcast)**2 + (v_data_forecast-v_data_hindcast)**2).mean(axis=(1,2))
    return RMSE_speed
...
def calc_abs_angle_difference(u_data_forecast, v_data_forecast, u_data_hindcast, v_data_hindcast):
    return np.abs(np.arctan2(v_data_hindcast, u_data_hindcast) - np.arctan2(v_data_forecast, u_data_forecast)).mean(axis=(1,2))


def calc_vector_corr_over_time(u_data_forecast, v_data_forecast, u_data_hindcast, v_data_hindcast):
    # run it over a for loop
    vec_corr_over_time = []
    for time_idx in range(u_data_forecast.shape[0]):
        vec_corr = calc_vector_correlation(u_data_forecast[time_idx, ...], v_data_forecast[time_idx, ...],
                                           u_data_hindcast[time_idx, ...], v_data_hindcast[time_idx, ...])
