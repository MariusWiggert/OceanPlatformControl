...
def check_feasibility_2D_w_sim(problem, T_hours_forward, deg_around_xt_xT_box, grid_res=None,
                               conv_m_to_deg=111120, hours_to_hj_solve_timescale=3600, sim_dt=600,
    ...
    sim_config_dict = {
        ...
        't_horizon_sim': math.ceil(T_hours_forward/24), # time horizon for data sub-setting in days
    }
    control_config_dict = {
        'PlannerConfig': {
        ...
        'SteeringCtrlConfig': {
            'name': 'None',
            'dt_replanning': 360000000000.}
    }

    # Step 2: Run the simulator
    sim = ocean_navigation_simulator.simulator.OceanNavSimulator(sim_config_dict=sim_config_dict,
                            control_config_dict=control_config_dict,
                            problem=problem)
    ...
    if sim.termination_reason != 'goal_reached':
        ...
        T_earliest_in_h = (sim.trajectory[3,-1] - sim.trajectory[3,0])/hours_to_hj_solve_timescale
...
def check_feasibility2D(problem, T_hours_forward, deg_around_xt_xT_box,
                             progress_bar=False, conv_m_to_deg=111120,
                        hours_to_hj_solve_timescale=3600, grid_res=[0.04, 0.04]):
