...
from ocean_navigation_simulator.environment.PlatformState import SpatioTemporalPoint, PlatformState
from ocean_navigation_simulator.environment.data_sources.SolarIrradiance.SolarIrradianceSource import *
from ocean_navigation_simulator.environment.data_sources.SeaweedGrowth.SeaweedFunction import *
...
class SeaweedGrowthGEOMAR(SeaweedGrowthSource, AnalyticalSource):
    ...
    def set_casadi_function(self):
        ...
        grid = [
            units.posix_to_rel_seconds_in_year(units.get_posix_time_from_np64(self.DataArray.coords['time'].values)),
        ...
        r_growth_wo_irradiance = ca.interpolant(
            'r_growth_wo_irradiance', 'linear', grid,
            self.DataArray['R_growth_wo_Irradiance'].fillna(0).values.ravel(order='F'))
        r_resp = ca.interpolant('r_resp', 'linear', grid, self.DataArray['R_resp'].fillna(0).values.ravel(order='F'))
        ...
        sym_NGR_per_second = r_growth_wo_irradiance(sym_rel_year_input) / (24 * 3600) * sym_f_Irradiance - r_resp(
            sym_rel_year_input) / (24 * 3600)
    ...
    def plot_R_growth_wo_Irradiance(self, time: datetime.datetime):
        ...
        Map_to_plot = self.DataArray['R_growth_wo_Irradiance'].interp(time=np.datetime64(time.replace(tzinfo=None)))
        Map_to_plot.plot()
