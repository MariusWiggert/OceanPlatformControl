...
class OceanFeatureConstructor(FeatureConstructor):
    ...
    def get_observation_space(config):
        measurement_space = gym.spaces.Box(
            low=-float("inf"), high=float("inf"), shape=(config["num_measurements"], 4)
        )
        ...
        return (
            gym.spaces.Tuple([measurement_space, ttr_space])
            if config["num_measurements"]
            else ttr_space
        )

    def get_features_from_state(
        self, observation: ArenaObservation, problem: NavigationProblem
        ...
        ttr_map = self.planner.interpolate_value_function_in_hours(
            ...
            width=self.config["ttr"]["xy_width_points"],
        )
        # if self.verbose > 0:
