...
class  OceanFeatureConstructor(FeatureConstructor):
    ...
    def get_observation_space(config):
        ...
        # Step 2: Local Map
        if config['local_map']:
            features += [OceanFeatureConstructor.get_map_space_from_config(config=config['local_map'])]

        # Step 3: Global Map
        if config['global_map']:
            features += [OceanFeatureConstructor.get_map_space_from_config(config=config['global_map'])]
    ...
    def get_map_space_from_config(config):
        map_features = (config['features']['ttr_forecast']
                        + config['features']['ttr_hindcast']
                        + len(config['features']['observer']['variables']) * len(config['features']['observer']['time'])
    ...
    def get_map_from_state(self, fc_obs, hc_obs, problem, config):
        ...
        if len(config['features']['observer']['variables']) > 0:
            t_interval = [hc_obs.platform_state.date_time + datetime.timedelta(hours=h) for h in config['features']['observer']['time']]
            ...
            gp_map = self.observer.get_data_over_area(
                ...
                method='linear'
            )[config['features']['observer']['variables']].to_array().to_numpy().astype('float32')
            gp_map = np.moveaxis(gp_map, [0, 1, 2, 3], [2, 3, 0, 1]).reshape((config['xy_width_points'],config['xy_width_points'],-1))
        ...
        if len(config['features']['currents_hindcast']) > 0:
            ...
            current_map = hc_obs.forecast_data_source.get_data_over_area(
                ...
                lat=np.linspace(y_interval[0], y_interval[1], config['xy_width_points']),
                method='linear'
            )['water_u', 'water_v'].to_array().to_numpy().astype('float32')
            current_map = np.moveaxis(current_map, [0, 1, 2, 3], [2, 3, 0, 1]).reshape((config['xy_width_points'],config['xy_width_points'],-1))
        ...
        if len(config['features']['true_error']) > 0:
            t_interval = [fc_obs.platform_state.date_time + datetime.timedelta(hours=h) for h in config['features']['true_error']]
            current_fc = fc_obs.forecast_data_source.get_data_over_area(
                x_interval=x_interval,
                y_interval=y_interval,
                t_interval=[t_interval[0], t_interval[-1]],
            ).interp(
                ...
                lat=np.linspace(y_interval[0], y_interval[1], config['xy_width_points']),
                method='linear'
            ).to_array().to_numpy().astype('float32')
            current_fc = np.moveaxis(current_fc, [0, 1, 2, 3], [2, 3, 0, 1]).reshape((config['xy_width_points'],config['xy_width_points'],-1))
            current_hc = hc_obs.forecast_data_source.get_data_over_area(
                x_interval=x_interval,
                y_interval=y_interval,
                t_interval=[t_interval[0], t_interval[-1]],
            ).interp(
            ...
            current_hc = np.moveaxis(current_hc, [0, 1, 2, 3], [2, 3, 0, 1]).reshape((config['xy_width_points'],config['xy_width_points'],-1))
