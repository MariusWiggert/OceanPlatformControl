...
class RLRunner:
    def __init__(
        ...
        json.dump(
            ...
            open(f"{self.config_folder}feature_constructor_config.json", "w"),
            indent=4,
        )
        json.dump(self.model_config, open(f"{self.config_folder}model_config.json", "w"), indent=4)
        json.dump(
            self.reward_function_config,
            open(f"{self.config_folder}reward_function_config.json", "w"),
            indent=4,
        )
        ...
        self.agent = agent_class(
            self.agent_config,
            logger_creator=lambda config: UnifiedLogger(config, self.results_folder, loggers=None),
        )
    ...
    def print_result(self, result, iteration, iterations):
        print(
            f'--------- Iteration {iteration} (Total Samples: {result["info"]["num_env_steps_trained"]}) ---------'
        )
        ...
        print(
            f'-- Episode Length [Min: {min(episodes_this_iteration):.0f}, Mean: {result["episode_len_mean"]:.1f}, Max: {max(episodes_this_iteration):.0f}] --'
        )
        ...
        print(
            f"Iteration Time: {train_time/60:.2f}min ({iterations * (train_time) / 60:.1f}min for {iterations} iterations, {(iterations - iteration) * (train_time) / 60:.1f}min to go)"
        )
