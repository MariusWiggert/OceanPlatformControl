...
class MissionGenerator:
    def __init__(
        ...
        self.timestamp_start = (self.config['t_range'][0] + self.config['problem_timeout']).timestamp()
    ...
    def generate_batch(self, batch_size: int) -> [NavigationProblem]:
        with timing(f'MissionGenerator: Batch of {batch_size} created ({{:.1f}}s)', self.verbose-1):
    ...
    def generate_target(self) -> Union[PlatformState, bool]:
        ...
        target = PlatformState(
            lon=units.Distance(deg=self.random.uniform(self.config['x_range'][0].deg, self.config['x_range'][1].deg)),
            lat=units.Distance(deg=self.random.uniform(self.config['y_range'][0].deg, self.config['y_range'][1].deg)),
            date_time=datetime.datetime.fromtimestamp(self.random.uniform(self.timestamp_start, self.timestamp_end).item(), tz=datetime.timezone.utc)
        )
        ...
        try:
            ...
            if self.verbose > 0:
                print(f'MissionGenerator: {bcolors.FAIL}Target aborted because of missing files.{bcolors.ENDC}')
                print(f"x:{self.config['x_range']}, y:{self.config['y_range']}, t:{[start_state.date_time, target.date_time]}")
                print(e)
            return False
        except ValueError as e:
            print(f'MissionGenerator: {bcolors.FAIL}Target aborted because of corrupted files.{bcolors.ENDC}')
        ...
        distance_to_shore = self.arena.ocean_field.hindcast_data_source.distance_to_land(target.to_spatial_point())
        if distance_to_shore.deg < self.config['target_distance_from_land']:
            if self.verbose > 0:
                print(f'MissionGenerator: {bcolors.FAIL}Target aborted because too close to land ({distance_to_shore}).{bcolors.ENDC}')
                print(f"x:{self.config['x_range']}, y:{self.config['y_range']}, t:{[start_state.date_time, target.date_time]}")
            return False
        ...
        if self.verbose > 1:
            print(f'MissionGenerator: {bcolors.OKGREEN}Target created ({time.time()-start:.1f}s) {target.to_spatio_temporal_point()}.{bcolors.ENDC}')
    ...
    def generate_starts(self, amount, silent=False) -> Union[List[SpatioTemporalPoint], bool]:
        ...
        if self.verbose > 0 and not silent:
            if len(points) < amount:
                print(f'MissionGenerator: There are only {len(points)} valid reachable points regarding mission time and minimum distance available but {amount} requested.')
            elif self.verbose > 1:
                print(f'MissionGenerator: {len(points)} starts created ({time.time()-start_time:.1f}s)')
    ...
    def sample_from_reachable_coordinates(
        ...
        all_values_dim = (planner.all_values[0] - planner.all_values.min()) * (planner.current_data_t_T - planner.current_data_t_0)
        reachable_condition = (reach_interval[0] < all_values_dim) & (all_values_dim < reach_interval[1])
        min_distance_condition = planner.initial_values > (min_distance / planner.characteristic_vec[0])
    ...
    def plot_batch(self, batch_size: int, filename: str, random_sample_points: Optional[int] = 10):
        ...
        def add_drawing(ax: plt.axis, rel_time_in_seconds):
            ...
            # Planner Frame
            ax.add_patch(patches.Rectangle(
                (self.hindcast_planner.grid.domain.lo[0], self.hindcast_planner.grid.domain.lo[1]),
                (self.hindcast_planner.grid.domain.hi[0] - self.hindcast_planner.grid.domain.lo[0]),
                (self.hindcast_planner.grid.domain.hi[1] - self.hindcast_planner.grid.domain.lo[1]),
            ...
            for problem in self.problems_archive[-batch_size:]:
                ax.scatter(
                    problem.start_state.lon.deg, problem.start_state.lat.deg,
                    facecolors='none', edgecolors='r', marker='o', label='starts'
                )
            ...
            if random_sample_points:
                for point in self.generate_starts(amount=random_sample_points, silent=True):
                    ax.scatter(
                        point.lon.deg, point.lat.deg,
                        facecolors='none', edgecolors='black', marker='o', label='possible sample points'
                    )
        ...
        if self.verbose > 0:
            print(f'MissionGenerator: Batch of {batch_size} plotted ({time.time()-plot_start_time:.1f}s)')
