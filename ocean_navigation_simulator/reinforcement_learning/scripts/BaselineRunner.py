...
class BaselineRunner:
    def __init__(self, generation_folder: str, ray_options: dict, verbose: Optional[int] = 0):
        ...
        self.ray_results = ray.get(
            [
                ...
                ).remote(index=index, row=row, verbose=self.verbose)
    ...
    def run_baseline(index, row, verbose):
        env = OceanEnv(verbose=verbose - 1)
        env.reset()
