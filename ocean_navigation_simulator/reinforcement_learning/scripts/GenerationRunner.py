...
class GenerationRunner:
    def __init__(
        ...
        if verbose > 0:
            print(
                f"GenerationRunner: Generating {groups * batches_per_group} batches in {groups} groups"
            )
        self.ray_results = ray.get(
            [
                self.generate_batch.options(
                    num_cpus=ray_options["resources"]["CPU"],
                    num_gpus=ray_options["resources"]["GPU"],
                    max_retries=ray_options["max_retries"],
                    resources={
                        i: ray_options["resources"][i]
                        for i in ray_options["resources"]
                        if i != "CPU" and i != "GPU"
                    },
                ).remote(
    ...
    def generate_batch(
        ...
        try:
            ...
            if verbose > 0:
                print(
                    f"GenerationRunner: Finished Batch {batch} Group {group} (total: {time.time()-batch_start_time:.1f}s, generate: {generate_time:.1f}s, plotting: {plot_time:.1f}s)"
                )
