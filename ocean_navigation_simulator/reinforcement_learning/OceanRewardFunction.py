	...
	def get_reward(
		...
		if problem_status == 0:
			if self.config['delta_ttr_forecast'] > 0:
				prev_ttr_fc = self.forecast_planner.interpolate_value_function_in_hours(observation=prev_fc_obs).item()
				curr_ttr_fc = self.forecast_planner.interpolate_value_function_in_hours(observation=curr_fc_obs).item()
				reward += self.config['delta_ttr_forecast'] * (prev_ttr_fc - curr_ttr_fc)
			if self.config['delta_ttr_hindcast'] > 0:
				prev_ttr_hc = self.hindcast_planner.interpolate_value_function_in_hours(observation=prev_hc_obs).item()
				curr_ttr_hc = self.hindcast_planner.interpolate_value_function_in_hours(observation=curr_hc_obs).item()
