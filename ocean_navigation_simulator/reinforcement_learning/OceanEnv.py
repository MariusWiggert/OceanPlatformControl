...
class OceanEnv(gym.Env):
    ...
    def __init__(
        ...
        with cluster_utils.timing(f"OceanEnv[Worker {worker_index}]: Created ({{:.1f}}s)", verbose):
            ...
            self.observation_space = OceanFeatureConstructor.get_observation_space(
                self.config["feature_constructor_config"]
            )
            self.reward_range = OceanRewardFunction.get_reward_range(
                self.config["reward_function_config"]
            )
            self.problem_factory = FileMissionProblemFactory(
                seed=self.worker_index, csv_file=f'{self.config["generation_folder"]}problems.csv'
            )
    ...
    def reset(self) -> np.array:
        ...
        if self.steps is None or self.steps > 0:
            ...
            if self.config["fake"] == "naive":
                self.naive_controller = NaiveController(
                    problem=self.problem, verbose=self.verbose - 1
                )
            if self.verbose > 0:
                print(
                    "OceanEnv[Worker {w}, Reset {r}]: Reset to Group {g} Batch {b} Index {i} ({t:.1f}s)".format(
                        w=self.worker_index,
                        r=self.resets,
        ...
        return self.feature_constructor.get_features_from_state(
            observation=self.prev_obs, problem=self.problem
        )
    ...
    def step(
        ...
        try:
            ...
            problem_status = self.arena.problem_status(
                self.problem,
                check_inside=True,
                margin=self.config["feature_constructor_config"]["ttr"]["xy_width_degree"] / 2,
            )
            done = problem_status != 0
            features = self.feature_constructor.get_features_from_state(
                observation=observation if not done else self.prev_obs, problem=self.problem
            )
            reward = self.reward_function.get_reward(
                self.prev_obs,
                observation if not done else self.prev_obs,
                self.problem,
                problem_status,
            )
            ...
            if done:
                if self.verbose > 0:
                    ...
                    if self.config["render"]:
                        self.render()
                    print(
                        "OceanEnv[Worker {w}, Reset {r}]: Finished Group {g} Batch {b} Index {i} ({su}, {st} steps, {t:.1f}h, ∑ΔTTR {rew:.1f}h, %TTR {ttr:.1f}) (step Ø: {stt:.1f}ms, episode: {ep:.1f}s, reset: {res:.1f}s, render: {ren:.1f}s, Mem: {mem:,.0f}MB)".format(
                            w=self.worker_index,
                            r=self.resets,
                            g=self.problem.extra_info["group"],
                            b=self.problem.extra_info["batch"],
                ...
                if self.verbose > 1:
                    print(
                        "OceanEnv[Worker {w}, Reset {r}]: Step {st} @ {ph:.0f}h {pm:.0f}min (step: {t:.1f}ms, {mem:,.0f}MB)".format(
                            w=self.worker_index,
                            r=self.resets,
                            st=self.steps,
    ...
    def render(self, mode="human"):
        ...
        ax.get_figure().savefig(
            f'{self.results_folder}Reset {self.resets} Group {self.problem.extra_info["group"]} Batch {self.problem.extra_info["batch"]} Index {self.problem.extra_info["factory_index"]}.png'
        )
        plt.clf()
