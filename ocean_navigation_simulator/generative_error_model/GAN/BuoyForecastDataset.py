...
class BuoyForecastErrorNpy(Dataset):
    def __init__(
        self, fc_dir, buoy_dir, areas=("area1"), concat_len=1, dataset_names=None, transform=None
    ):
        """Note: use 24*8 for length because not all files have full 9 days."""
        self.fc_dir = fc_dir
        ...
        for area in list(areas):
            ...
            fc_file_paths, buoy_file_paths = get_time_matched_file_lists(
                fc_file_paths, buoy_file_paths
            )
    ...
    def __len__(self):
        return min(len(self.fc_file_paths), len(self.buoy_file_paths)) * (
...
def compute_mean_std(data):
    ...
    mean = np.array(
        [np.sum(file, axis=(0, 2, 3)) / np.sum(file != 0, axis=(0, 2, 3)) for file in data]
    ).mean(axis=0)
    ...
    std = np.sqrt(
        np.array(
            [
                np.sum((file - helper_mean) ** 2, axis=(0, 2, 3)) / (np.prod(file.shape) / 2)
                for file in data
            ]
        ).sum(axis=0)
        / len(data)
...
def main():
    ...
    dataset = BuoyForecastErrorNpy(
        fc_dir, gt_dir, areas=["area1"], dataset_names=("fc", "buoy"), transform=True
    )
