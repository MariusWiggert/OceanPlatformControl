from ocean_navigation_simulator.generative_error_model.variogram.VisualizeVariogram import (
    VisualizeVariogram,
)
...
def save_tuned_empirial_variogram_3d(
    vvis: VisualizeVariogram, view_range: List[int], file_path: str
):
...
def save_tuned_empirial_variogram_2d(
    vvis: VisualizeVariogram, view_range: List[int], file_path: str
):
    """Convert hand-tuned variogram to dataframe and save."""
...
def sample_from_xr(
    data: xr.Dataset, num_samples, variables: Tuple[str, str] = ("u_error", "v_error")
) -> pd.DataFrame:
