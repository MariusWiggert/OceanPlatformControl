...
class VariogramFitting:
    ...
    def fit_model(self, num_of_components, constrain_weighting=False):
        """Takes the lags and their associated values and fits multiple stacked gaussian models to it.
    ...
    def plot_sliced_variogram(self, var: str):
        ...
        lags = self.lags[:, lag_idx]
        # get data for fitted function line plot
        function_semivariance = self._get_sliced_fitted_function_data(var, lags)
        ...
        plt.plot(lags, function_semivariance, label="fitted function")
        plt.scatter(empirical_lags, empirical_semivariance, label="empirical points")
        plt.legend()
        plt.xlim(left=0)
        plt.ylim([0, 1.5])
        plt.xlabel(f"{var}")
    ...
    def _get_sliced_fitted_function_data(self, var: str, lags: List[float]):
