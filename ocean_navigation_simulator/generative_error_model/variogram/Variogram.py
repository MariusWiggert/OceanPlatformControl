...
from ocean_navigation_simulator.generative_error_model.variogram.IndexPairGenerator import (
    IndexPairGenerator,
)
...
class Variogram:
    ...
    def __init__(
        self, data: pd.DataFrame = None, variables: Tuple[str, str] = ("u_error", "v_error")
    ):
        self.data = data
        if data is not None:
    ...
    def detrend(self, detrend_var: AnyStr = "lat", num_bins: int = 1) -> Dict[str, List[float]]:
        ...
        self.data[f"{detrend_var}_bins"] = pd.cut(
            self.data[detrend_var], bins, labels=bin_labels, include_lowest=True
        )
        ...
        for bin_label in bin_labels:
            u_stats = []
            u_stats.append(
                self.data[self.data[f"{detrend_var}_bins"] == bin_label]["u_error"].mean()
            )
            u_stats.append(
                np.sqrt(self.data[self.data[f"{detrend_var}_bins"] == bin_label]["u_error"].var())
            )

            v_stats = []
            v_stats.append(
                self.data[self.data[f"{detrend_var}_bins"] == bin_label]["v_error"].mean()
            )
            v_stats.append(
                np.sqrt(self.data[self.data[f"{detrend_var}_bins"] == bin_label]["v_error"].var())
            )

            bin_statistics[bin_label] = {"u_error": u_stats, "v_error": v_stats}

        # make accessible for plotting function
        self.detrend_var, self.bin_labels, self.bin_statistics = (
            detrend_var,
            bin_labels,
            bin_statistics,
        )
        ...
        self.data["detrended_u_error"] = self.data.apply(
            lambda x: (x["u_error"] - bin_statistics[x[f"{detrend_var}_bins"]]["u_error"][0])
            / bin_statistics[x[f"{detrend_var}_bins"]]["u_error"][1],
            axis=1,
        )
        self.data["detrended_v_error"] = self.data.apply(
            lambda x: (x["v_error"] - bin_statistics[x[f"{detrend_var}_bins"]]["v_error"][0])
            / bin_statistics[x[f"{detrend_var}_bins"]]["v_error"][1],
            axis=1,
        )
    ...
    def build_variogram(
        self,
        res_tuple: Tuple[float],
        num_workers: int,
        chunk_size: int,
        cross_buoy_pairs_only: bool = True,
        detrended: bool = True,
        units: str = "km",
        is_3d: bool = True,
        logger: logging = None,
        ...
        while True:
            ...
            with iolock:
                ...
                if iteration % 10 == 0 and iteration != 0 and logger is not None:
                    logger.info(
                        f"Iteration: {iteration}, Estimated {round(100*(running_sum/number_of_pairs),2)}% of pairs finished."
                    )
                iteration += 1
        ...
        self.bins = (
            np.divide(
                self.bins, self.bins_count, out=np.zeros_like(self.bins), where=self.bins_count != 0
            )
            / 2
        )
        return self.bins, self.bins_count

    def _calculate_chunk_3d(
        self,
        q: mp.Queue,
        time: List[int],
        lon: List[int],
        ...
        while True:
            ...
            t_lag = np.floor(
                (np.absolute(time[idx_i] - time[idx_j]) / self.t_res).astype(float)
            ).astype(int)
    ...
    def _variogram_setup_3d(self, res_tuple: Tuple[float]):
        ...
        self.t_bins = np.ceil(
            (self.data["time_offset"].max() - self.data["time_offset"].min() + 1) / self.t_res
        ).astype(int)
    ...
    def _calculate_chunk_2d(
        ...
        while True:
            ...
            if self.units == "degrees":
                ...
                space_lag = np.floor(np.sqrt(lon_lag**2 + lat_lag**2) / self.space_res).astype(
                    int
                )
    ...
    def _variogram_setup_2d(self, res_tuple: Tuple[float]):
        ...
        if self.units == "degrees":
            ...
            self.space_bins = np.ceil(
                (np.sqrt(max_lon_lag**2 + max_lat_lag**2) + 1) / self.space_res
            ).astype(int)
        elif self.units == "km":
            max_y_lag = (110.574 * max_lat) - (110.574 * min_lat)
            max_x_lag = (111.320 * max_lon * np.cos(0)) - (111.320 * min_lon * np.cos(0))
            self.space_bins = np.ceil(
                (np.sqrt(max_y_lag**2 + max_x_lag**2) / self.space_res) + 1
            ).astype(int)
    ...
    def mask_pairs_from_same_buoy(self, indices, buoy_vector):
        ...
        mask_idx = np.where(mask_same_buoy is True)  # need == here!
    ...
    def plot_detrended_bins(self) -> None:
        ...
        for i, bin_label in enumerate(self.bin_labels):
            detrended_u_error = self.data[self.data[f"{self.detrend_var}_bins"] == bin_label][
                "detrended_u_error"
            ].to_numpy()
            detrended_v_error = self.data[self.data[f"{self.detrend_var}_bins"] == bin_label][
                "detrended_v_error"
            ].to_numpy()
            ...
            df_temp = pd.DataFrame(
                {
                    ...
                    "u_error_detrended": detrended_u_error,
            ...
            if len(self.bin_labels) == 1:
                axs.plot(
                    detrended_group["time"],
                    detrended_group["u_error_detrended"],
                    label="u error binned and detrended",
                )
                axs.plot(
                    detrended_group["time"],
                    detrended_group["v_error_detrended"],
                    label="v error binned and detrended",
                )
                axs.title.set_text(
                    f"Plot for '{self.detrend_var}' range of {bin_label} degrees (no binning)"
                )
                ...
                axs[i].set_xticks(
                    np.arange(0, len(detrended_group), round(len(detrended_group) / 8))
                )
                axs[i].grid()
        plt.legend()
        plt.show()
...
def convert_degree_to_km(pts1: np.ndarray, pts2: np.ndarray) -> List[np.ndarray]:
    ...
    if len(pts1.shape) > 1:
        dx = (
            (pts1[:, 0] - pts2[:, 0])
            * 40075.2
            * np.cos((pts1[:, 1] + pts2[:, 1]) * np.pi / 360)
            / 360
        )
