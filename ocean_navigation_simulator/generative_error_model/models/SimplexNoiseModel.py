...
class SimplexNoiseModel:
    ...
    def get_noise(self, x: np.ndarray, y: np.ndarray, elapsed_time: np.ndarray) -> np.ndarray:
        ...
        # output noise is of shape [time, lat, lon], since it gets reversed in the opensimlex method
...
def main():
    ...
    output = noise.get_noise(np.array([200, 300, 500, 500]), np.array([300, 200, 100, 200, 300]), elapsed_times)
    print(output.shape)
