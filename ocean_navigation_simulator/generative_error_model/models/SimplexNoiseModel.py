...
class CurrentVector(object):
    ...
    def add(self, other: "CurrentVector") -> "CurrentVector":
        if not isinstance(other, CurrentVector):
            raise NotImplementedError(f"Cannot add CurrentVector with {type(other)}")
...
class SimplexNoiseModel:
    ...
    def __init__(
        self, u_comp_harmonics: List[HarmonicParameters], v_comp_harmonics: List[HarmonicParameters]
    ):
    ...
    def get_noise(self, x: np.ndarray, y: np.ndarray, elapsed_time: np.ndarray) -> np.ndarray:
        ...
        noise = np.moveaxis(
            noise, [*range(1, len(noise.shape)), 0], list(range(0, len(noise.shape)))
        )
        return noise
...
class NoisyCurrentComponent:
    ...
    def __init__(self, comp_harmonics: List[HarmonicParameters]):
        ...
        self._harmonics = [NoisyCurrentHarmonic(params) for params in harmonic_params]
...
class NoisyCurrentHarmonic:
    ...
    def get_noise(self, x: np.ndarray, y: np.ndarray, elapsed_times: np.ndarray) -> np.ndarray:
        ...
        time_in_hours = np.array(
            [
                elapsed_time.astype("timedelta64[h]") / np.timedelta64(1, "h")
...
def test():
    ...
    elapsed_times = np.array(
        [np.timedelta64(5, "h"), np.timedelta64(6, "h"), np.timedelta64(20, "h")]
    )
    output = noise.get_noise(
        np.array([200, 300, 500, 550]), np.array([300, 200, 100, 200, 300]), elapsed_times
    )
