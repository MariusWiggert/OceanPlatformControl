...
for key, dims in resize_mapping.items():
    ...
    TABLE_1_DICT[key] = np.append(TABLE_1_DICT[key], np.zeros((new_rows, 3)), axis=0)
...
HELIO_LAT_TABLE = np.array([TABLE_1_DICT["B0"], TABLE_1_DICT["B1"]])
...
def heliocentric_longitude(jme):
    ...
    for row in range(HELIO_LONG_TABLE.shape[1]):
        l0 += HELIO_LONG_TABLE[0, row, 0] * np.cos(
            HELIO_LONG_TABLE[0, row, 1] + HELIO_LONG_TABLE[0, row, 2] * jme
        )
        l1 += HELIO_LONG_TABLE[1, row, 0] * np.cos(
            HELIO_LONG_TABLE[1, row, 1] + HELIO_LONG_TABLE[1, row, 2] * jme
        )
        l2 += HELIO_LONG_TABLE[2, row, 0] * np.cos(
            HELIO_LONG_TABLE[2, row, 1] + HELIO_LONG_TABLE[2, row, 2] * jme
        )
        l3 += HELIO_LONG_TABLE[3, row, 0] * np.cos(
            HELIO_LONG_TABLE[3, row, 1] + HELIO_LONG_TABLE[3, row, 2] * jme
        )
        l4 += HELIO_LONG_TABLE[4, row, 0] * np.cos(
            HELIO_LONG_TABLE[4, row, 1] + HELIO_LONG_TABLE[4, row, 2] * jme
        )
        l5 += HELIO_LONG_TABLE[5, row, 0] * np.cos(
            HELIO_LONG_TABLE[5, row, 1] + HELIO_LONG_TABLE[5, row, 2] * jme
        )
    ...
    return ca.mod(l_intermediate, 360)
...
def heliocentric_latitude(jme):
    ...
    for row in range(HELIO_LAT_TABLE.shape[1]):
        b0 += HELIO_LAT_TABLE[0, row, 0] * np.cos(
            HELIO_LAT_TABLE[0, row, 1] + HELIO_LAT_TABLE[0, row, 2] * jme
        )
        b1 += HELIO_LAT_TABLE[1, row, 0] * np.cos(
            HELIO_LAT_TABLE[1, row, 1] + HELIO_LAT_TABLE[1, row, 2] * jme
        )
...
def heliocentric_radius_vector(jme):
    ...
    for row in range(HELIO_RADIUS_TABLE.shape[1]):
        r0 += HELIO_RADIUS_TABLE[0, row, 0] * np.cos(
            HELIO_RADIUS_TABLE[0, row, 1] + HELIO_RADIUS_TABLE[0, row, 2] * jme
        )
        r1 += HELIO_RADIUS_TABLE[1, row, 0] * np.cos(
            HELIO_RADIUS_TABLE[1, row, 1] + HELIO_RADIUS_TABLE[1, row, 2] * jme
        )
        r2 += HELIO_RADIUS_TABLE[2, row, 0] * np.cos(
            HELIO_RADIUS_TABLE[2, row, 1] + HELIO_RADIUS_TABLE[2, row, 2] * jme
        )
        r3 += HELIO_RADIUS_TABLE[3, row, 0] * np.cos(
            HELIO_RADIUS_TABLE[3, row, 1] + HELIO_RADIUS_TABLE[3, row, 2] * jme
        )
        r4 += HELIO_RADIUS_TABLE[4, row, 0] * np.cos(
            HELIO_RADIUS_TABLE[4, row, 1] + HELIO_RADIUS_TABLE[4, row, 2] * jme
        )
...
def longitude_nutation(julian_ephemeris_century, x0, x1, x2, x3, x4):
    ...
    for row in range(NUTATION_YTERM_ARRAY.shape[0]):
        ...
        term = (a + b * julian_ephemeris_century) * np.sin(radians(argsin))
        delta_psi_sum += term
...
def obliquity_nutation(julian_ephemeris_century, x0, x1, x2, x3, x4):
    ...
    for row in range(NUTATION_YTERM_ARRAY.shape[0]):
        ...
        term = (c + d * julian_ephemeris_century) * np.cos(radians(argcos))
        delta_eps_sum += term
...
def apparent_sidereal_time(mean_sidereal_time, longitude_nutation, true_ecliptic_obliquity):
    v = mean_sidereal_time + longitude_nutation * np.cos(radians(true_ecliptic_obliquity))
    return v
...
def geocentric_sun_right_ascension(
    ...
    num = np.sin(radians(apparent_sun_longitude)) * np.cos(
        radians(true_ecliptic_obliquity)
    ) - np.tan(radians(geocentric_latitude)) * np.sin(radians(true_ecliptic_obliquity))
    alpha = degrees(np.arctan2(num, np.cos(radians(apparent_sun_longitude))))
    # TODO: check
...
def geocentric_sun_declination(
    ...
    delta = degrees(
        np.arcsin(
            np.sin(radians(geocentric_latitude)) * np.cos(radians(true_ecliptic_obliquity))
            + np.cos(radians(geocentric_latitude))
            * np.sin(radians(true_ecliptic_obliquity))
            * np.sin(radians(apparent_sun_longitude))
        )
    )
    return delta
...
def uterm(observer_latitude):
    u = np.arctan(0.99664719 * np.tan(radians(observer_latitude)))
    return u


def xterm(u, observer_latitude, observer_elevation):
    x = np.cos(u) + observer_elevation / 6378140 * np.cos(radians(observer_latitude))
    return x


def yterm(u, observer_latitude, observer_elevation):
    y = 0.99664719 * np.sin(u) + observer_elevation / 6378140 * np.sin(radians(observer_latitude))
    return y
...
def parallax_sun_right_ascension(
    ...
    num = (
        -xterm * np.sin(radians(equatorial_horizontal_parallax)) * np.sin(radians(local_hour_angle))
    )
    denom = np.cos(radians(geocentric_sun_declination)) - xterm * np.sin(
        radians(equatorial_horizontal_parallax)
    ) * np.cos(radians(local_hour_angle))
    delta_alpha = degrees(np.arctan2(num, denom))
    return delta_alpha
...
def topocentric_sun_declination(
    ...
    num = (
        np.sin(radians(geocentric_sun_declination))
        - yterm * np.sin(radians(equatorial_horizontal_parallax))
    ) * np.cos(radians(parallax_sun_right_ascension))
    denom = np.cos(radians(geocentric_sun_declination)) - xterm * np.sin(
        radians(equatorial_horizontal_parallax)
    ) * np.cos(radians(local_hour_angle))
    delta = degrees(np.arctan2(num, denom))
    return delta
...
def topocentric_elevation_angle_without_atmosphere(
    ...
    e0 = degrees(
        np.arcsin(
            np.sin(radians(observer_latitude)) * np.sin(radians(topocentric_sun_declination))
            + np.cos(radians(observer_latitude))
            * np.cos(radians(topocentric_sun_declination))
            * np.cos(radians(topocentric_local_hour_angle))
        )
    )
    return e0
...
def topocentric_astronomers_azimuth(
    ...
    num = np.sin(radians(topocentric_local_hour_angle))
    denom = np.cos(radians(topocentric_local_hour_angle)) * np.sin(
        radians(observer_latitude)
    ) - np.tan(radians(topocentric_sun_declination)) * np.cos(radians(observer_latitude))
...
def equation_of_time(
    sun_mean_longitude, geocentric_sun_right_ascension, longitude_nutation, true_ecliptic_obliquity
):
    E = (
        sun_mean_longitude
        - 0.0057183
...
def solar_position(
    ...
    result = do_calc(
        unixtime, lat, lon, elev, pressure, temp, delta_t, atmos_refract, numthreads, sst, esd
    )
...
def transit_sunrise_sunset(dates, lat, lon, delta_t, numthreads):
    ...
    cos_arg = (np.sin(radians(-0.8333)) - np.sin(radians(lat)) * np.sin(radians(ttday0_res[2]))) / (
        np.cos(radians(lat)) * np.cos(radians(ttday0_res[2]))
    )
    cos_arg[abs(cos_arg) > 1] = np.nan
    H0 = degrees(np.arccos(cos_arg)) % 180
    ...
    h = degrees(
        np.arcsin(
            np.sin(radians(lat)) * np.sin(radians(delta_prime))
            + np.cos(radians(lat)) * np.cos(radians(delta_prime)) * np.cos(radians(Hp))
        )
    )
    ...
    R = (
        m[1]
        + (h[1] + 0.8333)
        / (360 * np.cos(radians(delta_prime[1])) * np.cos(radians(lat)) * np.sin(radians(Hp[1])))
    ) * 86400
    S = (
        m[2]
        + (h[2] + 0.8333)
        / (360 * np.cos(radians(delta_prime[2])) * np.cos(radians(lat)) * np.sin(radians(Hp[2])))
    ) * 86400
