...
def geocentric_sun_right_ascension(apparent_sun_longitude,
                                   true_ecliptic_obliquity,
                                   geocentric_latitude):
    num = (np.sin(radians(apparent_sun_longitude))
           * np.cos(radians(true_ecliptic_obliquity))
           - np.tan(radians(geocentric_latitude))
...
def geocentric_sun_declination(apparent_sun_longitude, true_ecliptic_obliquity,
                               geocentric_latitude):
    delta = degrees(np.arcsin(np.sin(radians(geocentric_latitude)) *
                                 np.cos(radians(true_ecliptic_obliquity)) +
...
def yterm(u, observer_latitude, observer_elevation):
    y = (0.99664719 * np.sin(u) + observer_elevation / 6378140
         * np.sin(radians(observer_latitude)))
    return y


def parallax_sun_right_ascension(xterm, equatorial_horizontal_parallax,
                                 local_hour_angle, geocentric_sun_declination):
    num = (-xterm * np.sin(radians(equatorial_horizontal_parallax))
           * np.sin(radians(local_hour_angle)))
    denom = (np.cos(radians(geocentric_sun_declination))
             - xterm * np.sin(radians(equatorial_horizontal_parallax))
...
def topocentric_sun_declination(geocentric_sun_declination, xterm, yterm,
                                ...
                                parallax_sun_right_ascension,
                                local_hour_angle):
    num = ((np.sin(radians(geocentric_sun_declination)) - yterm
            * np.sin(radians(equatorial_horizontal_parallax)))
           * np.cos(radians(parallax_sun_right_ascension)))
    denom = (np.cos(radians(geocentric_sun_declination)) - xterm
             * np.sin(radians(equatorial_horizontal_parallax))
             * np.cos(radians(local_hour_angle)))
...
def topocentric_elevation_angle_without_atmosphere(observer_latitude,
    ...
    e0 = degrees(np.arcsin(
        np.sin(radians(observer_latitude))
        * np.sin(radians(topocentric_sun_declination))
        + np.cos(radians(observer_latitude))
...
def topocentric_astronomers_azimuth(topocentric_local_hour_angle,
    ...
    denom = (np.cos(radians(topocentric_local_hour_angle))
             * np.sin(radians(observer_latitude))
...
def transit_sunrise_sunset(dates, lat, lon, delta_t, numthreads):
    ...
    cos_arg = ((np.sin(radians(-0.8333)) - np.sin(radians(lat))
               * np.sin(radians(ttday0_res[2]))) /
    ...
    h = degrees(np.arcsin(np.sin(radians(lat)) *
                             np.sin(radians(delta_prime)) +
                             np.cos(radians(lat)) *
                             np.cos(radians(delta_prime))
                             * np.cos(radians(Hp))))

    T = (m[0] - Hp[0] / 360) * 86400
    R = (m[1] + (h[1] + 0.8333) / (360 * np.cos(radians(delta_prime[1])) *
                                   np.cos(radians(lat)) *
                                   np.sin(radians(Hp[1])))) * 86400
    S = (m[2] + (h[2] + 0.8333) / (360 * np.cos(radians(delta_prime[2])) *
                                   np.cos(radians(lat)) *
                                   np.sin(radians(Hp[2])))) * 86400
