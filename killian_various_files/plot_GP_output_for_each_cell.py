...
if plot_fixed:
    file_025, file_05, file_1 = (
        path + "config_GP_025_12_fixed_export_4.pickle",
        path + "config_GP_05_12_fixed_export_4.pickle",
        path + "config_GP_1_12_fixed_export_4.pickle",
    )
else:
    file_025, file_05, file_1 = (
        path + "config_GP_025_12_export_4.pickle",
        path + "config_GP_05_12_export_4.pickle",
        path + "config_GP_1_12_export_4.pickle",
    )

with open(file_025, "rb") as handle_025, open(file_05, "rb") as handle_05, open(
    file_1, "rb"
) as handle_1:
    ...
    rmse_improved = (
        np.array(file_05["rmse_improved_all_lags_and_radius"]),
        "rmse_improved",
    )
    ...
    for key in results_grids_05.keys():
        if key.startswith(metric_to_plot):
            ...
            for i, to_plot in enumerate(to_plots[1:2]):
                ...
                X, Y = np.meshgrid(
                    range(to_plot.shape[0]), np.arange(0, to_plot.shape[1] / 12, 1 / 12)
                )  # `plot_surface` expects `x` and `y` data to be 2D
                ha.plot_surface(X.T, Y.T, to_plot, color=colors[i])
                fake2Dlines.append(
                    matplotlib.lines.Line2D([0], [0], linestyle="none", c=colors[i], marker="o")
                )
                if plot_2d:
...
path_val = "ablation_study/export_all_results_validation_set/"
path_test = "ablation_study/export_all_results_testing_set/"
