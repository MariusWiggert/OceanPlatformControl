...
start = datetime.datetime(2022, 5, 3, 12, 30, tzinfo=datetime.timezone.utc) + datetime.timedelta(hours=1)
...
for i in range(len_days):
    if not i % 10:
        print(i + 1, "/", len_days)
    try:
        f = fc.get_data_over_area(lon_interv, lat_interv,
                                  [start, start + delta + delta_lags]).isel(
        ...
        h = hc.get_data_over_area(lon_interv_hc, lat_interv_hc, [start - margin, start + delta_lags + margin])
        ...
        f_renamed = xr.merge([f, f.rename_vars(water_u="initial_forecast_u", water_v="initial_forecast_v")])
...
for dim_current in range(n_dims):
    ...
    plt.subplots_adjust(hspace=.348, wspace=.229)
    plt.show()
...
dims_to_plot = ["u", "v", "magnitude"]
fig, ax = plt.subplots(3, 1, sharex=True, sharey=True)
fig.suptitle(f'Error scatter plot')
for i, dim in enumerate(dims_to_plot):
...
stat_lags = []
for i in range(len(array_error_per_time)):
    stat_lags.append(pd.DataFrame(np.array([abs(e[i]).flatten() for e in dims_first]).T,
