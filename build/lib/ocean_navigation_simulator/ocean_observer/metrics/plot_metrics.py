...
def plot_r2(metrics_dict: Dict, ax: plt.axes, X_LABELSIZE=10):
    time_axis = [datetime.datetime.fromtimestamp(time, tz=datetime.timezone.utc) for time in metrics_dict["time"]]
...
def plot_rmse(metrics_dict: Dict, ax: plt.axes, X_LABELSIZE=10):
    time_axis = [datetime.datetime.fromtimestamp(time, tz=datetime.timezone.utc) for time in metrics_dict["time"]]
    format_datetime_x_axis(ax)
    ax.plot(time_axis, metrics_dict["rmse_improved"], label="improved",
            linewidth=0.5)
    ax.plot(time_axis, metrics_dict["rmse_initial"], label="initial",
...
def plot_vector_correlation(metrics_dict: Dict, ax: plt.axes, X_LABELSIZE=10):
    ...
    time_axis = [datetime.datetime.fromtimestamp(time, tz=datetime.timezone.utc) for time in metrics_dict["time"]]
    format_datetime_x_axis(ax)
    ax.plot(time_axis, metrics_dict["vector_correlation_improved"], label="improved",
            linewidth=0.5)
    ax.plot(time_axis, metrics_dict["vector_correlation_initial"], label="initial",
            linewidth=0.5, linestyle='--', color='black')
