...
lon_left, lon_right = (
    file["longitude"].min().item(),
    file["longitude"].max().item(),
)  # -28.99999999999986, - 23.000000000000014  # -62, -12
...
end_date = datetime.datetime(2022, 5, 7, 12, 00, 00)
...
def generate_end_point(initial_point: SpatialPoint) -> SpatialPoint:
    ...
    new_point = SpatialPoint(
        Distance(deg=initial_point.lon.deg + math.cos(angle) * distance_start_end_point.deg),
...
def get_random_dates_among_list(dates, number_dates, duration_simulation):
    ...
    return [
        generate_random_date(*pair, duration_simulation)
        for pair in dates[np.random.choice(range(len(sec)), number_dates, p=sec / sec.sum())]
    ]
...
def generate_temporal_points_within_boundaries(
    numbers: int, all_intervals: List, end_included=False
) -> SpatioTemporalPoint:
    ...
    return [
        SpatioTemporalPoint(lat=point.lat, lon=point.lon, date_time=date)
...
def check_point(point: SpatialPoint):
    ...
    """
    return (
        lat_bottom <= point.lat.deg <= lat_top and lon_left <= point.lon.deg <= lon_right and True
    )  # check_if_square_margin_to_shore()
...
def check_if_square_margin_to_shore(prob, point, margin_to_shore):
    ...
    x_low_idx = (
        np.abs(prob.hindcast_data_source["grid_dict"]["x_grid"] - (point[0] - margin_to_shore))
    ).argmin()
    x_high_idx = (
        np.abs(prob.hindcast_data_source["grid_dict"]["x_grid"] - (point[0] + margin_to_shore))
    ).argmin()
    y_low_idx = (
        np.abs(prob.hindcast_data_source["grid_dict"]["y_grid"] - (point[1] - margin_to_shore))
    ).argmin()
    y_high_idx = (
        np.abs(prob.hindcast_data_source["grid_dict"]["y_grid"] - (point[1] + margin_to_shore))
    ).argmin()
    ...
    contains_land = np.any(
        prob.hindcast_data_source["grid_dict"]["spatial_land_mask"][
            y_low_idx:y_high_idx, x_low_idx:x_high_idx
        ]
    )
...
def point_to_dict(
    point: Union[SpatialPoint, SpatioTemporalPoint], radius_in_m: int = 1
) -> dict[str, any]:
    return {"lon_in_deg": point.lon.deg, "lat_in_deg": point.lat.deg} | (
        {"datetime": point.date_time.strftime("%Y-%m-%dT%H:%M:%SZ")}
        if isinstance(point, SpatioTemporalPoint)
        else {"radius_in_m": radius_in_m}
    )
...
def problems_to_yaml(problems: list[str], dir_name: str, filename: str, seed: int):
    ...
    with open(path, "w") as f:
        yaml.dump(
            {"problems": [problem_format(problem) for problem in problems]},
            f,
            sort_keys=False,
...
def check_area_to_have_no_nan():
    ...
    while np.isnan(
        lon_lat_xarray.sel(longitude=slice(lon_left - tile_radius, lon_right + tile_radius))
        .isel(latitude=len_lat // 2)
        .to_array()
        .to_numpy()
    ).sum():
    ...
    while np.isnan(
        lon_lat_xarray.sel(latitude=slice(lat_bottom - tile_radius, lat_top + tile_radius))
        .isel(longitude=len_lon // 2)
        .to_array()
        .to_numpy()
    ).sum():
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--num-problems", dest="problems", type=int, help="number of problems wanted"
    )
