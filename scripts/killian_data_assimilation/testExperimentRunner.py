...
def run_experiments_and_visualize_area(
    ...
    if folder_config_file is not None:
        ...
        exp = ExperimentRunner(yaml_file_config, filename_problems="all_problems_3")
    # coordinates to visualize

    exp.visualize_area(
        number_days_forecasts=number_forecasts_in_days,
        use_NN=use_NN,
...
if __name__ == "__main__":
    ...
    # Visualize results including the GP and the NN
        ...
        parser.add_argument("--config-file", type=str, help="file name")
        parser.add_argument(
            "--config-folder", type=str, help="folder where the yaml config file is located"
        )
        parser.add_argument(
            "--problems-folder", type=str, help="folder where the yaml problems file is located"
        )
        parser.add_argument("--number-days", type=int, default=5)
        parser.add_argument(
            "--display-forecast", default=True, action=argparse.BooleanOptionalAction
        )
        args = parser.parse_args()
