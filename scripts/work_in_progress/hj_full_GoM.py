...
arena = ArenaFactory.create(scenario_name="gulf_of_mexico_HYCOM_hindcast_full",
                            # t_interval=[
...
ax = arena.ocean_field.forecast_data_source.plot_data_at_time_over_area(
    ...
    x_interval=[-98, -77 ], y_interval=[18, 32],
    return_ax=True,
    # figsize=(12, 7),
    quiver_scale=30,
    quiver_spatial_res= 0.3,
...
planner.plot_reachability_animation(time_to_reach=False,
                                    granularity_in_h=10,
                                    temporal_resolution=3600*3, fps=20,
                                    with_opt_ctrl=False,
...
planner.plot_reachability_animation(time_to_reach=True,
                                    granularity_in_h=5,
                                    temporal_resolution=1800, fps=20,
                                    with_opt_ctrl=True,
...
arena.animate_trajectory(problem=problem,
                         set_title=True,
                         temporal_resolution=1800, full_traj=False, fps=15,
                         output="full_GoM.mp4",
                         traj_linestyle='-',
                         traj_color='black',
                         traj_linewidth=6,
...
planner.plot_reachability_animation(time_to_reach=True, granularity_in_h=5, with_opt_ctrl=False,
                                    temporal_resolution=3600, fps=15, filename="replanning_x_t_fmrc_idx_{}.mp4".format(current_fmrc.timestamp()),
                                    forward_time=True,
                                    add_drawing=plan_from_x_t_plot,
...
planner.plot_reachability_animation(time_to_reach=True, granularity_in_h=5, with_opt_ctrl=False,
                                    temporal_resolution=3600, filename="replanning_x_t_fmrc_idx_{}.mp4".format(current_fmrc.timestamp()),
                                    forward_time=True, add_drawing=plan_from_x_t_plot,
...
arena.animate_trajectory(problem=problem,
                         set_title=True,
                         temporal_resolution=3600, full_traj=False,
...
planner.plot_reachability_animation(time_to_reach=True,
                                    granularity_in_h=5,
                                    filename="fmrc_idx_{}.mp4".format(0))
#%% create an add_ax_function
import numpy as np
def plan_traj_from_x_t_and_plot(ax, rel_time, arena_traj=arena.state_trajectory, num_traj_disc=None, dt_in_sec=1800, **kwargs):
    # get x_t from arena trajectory
    idx_x_t = min(np.searchsorted(a=arena_traj[:, 2] - planner.current_data_t_0, v=rel_time), len(arena_traj)-1)
    ...
    times, x_traj, contr_seq, _ = planner._extract_trajectory(x_start=planner.get_x_from_full_state(x_t), t_start=x_t[2],
...
#%%
planner.plot_reachability_snapshot_over_currents(rel_time_in_seconds=3600*50,
                                   return_ax=False, time_to_reach=True,
                                   add_drawing=plan_traj_from_x_t_and_plot)
#%% Now make an animation with that..
planner.plot_reachability_animation(time_to_reach=True, granularity_in_h=5, with_opt_ctrl=False,
                                    temporal_resolution=3600*10, filename="test_reach_animation_w_ctrl.mp4",
                                    forward_time=True, add_drawing=plan_traj_from_x_t_and_plot)
#%%
