...
ratio,num_samples = .8, array_forecast.shape[0]
...
for i,time in enumerate(y_tr["time"]):
    ...
    for j,lat in enumerate(y_tr["lat"]):
        for k,long in enumerate(y_tr["lon"]):
            print("toto")
            print(lat,long,OceanCurrentVector(*y_tr[i,j,k].to_numpy()))
            print("titi")
            model.observe(lat,long,i,OceanCurrentVector(*y_tr[i,j,k].to_numpy()))
#%%
