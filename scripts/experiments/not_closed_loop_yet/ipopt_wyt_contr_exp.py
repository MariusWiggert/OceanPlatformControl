...
from src.tracking_controllers.simple_P_tracker import *
import os
...
# Step 2.2: initialize waypoint_tracker
P_tracker = simple_P_tracker(ipopt_planner.get_waypoints())
#%%
# Step 3: init the simulator
sim = Simulator(ipopt_planner, problem=prob, project_dir=project_dir, sim_config='simulator.yaml', wypt_contr=P_tracker)
#%%
# Step 4: run the simulator
sim.run(T_in_h=T_planner_in_h)
#%%
# Step 5: plot it
sim.plot_trajectory(gif_name='ipopt_var_cur', plotting_type='battery')
sim.plot_trajectory(gif_name='ipopt_var_cur', plotting_type='2D')
sim.plot_trajectory(gif_name='ipopt_var_cur', plotting_type='gif')