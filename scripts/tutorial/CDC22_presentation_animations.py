...
if download_data:
    # load yaml file
    import os, yaml
    with open(os.path.join("config/arena/", f"gulf_of_mexico_CDC22_animations.yaml")) as f:
    ...
    _ = ArenaFactory.create(scenario_config=config,
                            t_interval=[datetime.datetime(2021, 11, 20, 12, 0, tzinfo=datetime.timezone.utc),
...
x_0 = PlatformState(lon=units.Distance(deg=x_0_list[0]),lat=units.Distance(deg=x_0_list[1]),date_time=x_0_list[2])
...
ax = arena.ocean_field.forecast_data_source.plot_data_at_time_over_area(
    time=x_0.date_time, x_interval=plot_x_interval, y_interval=plot_y_interval, return_ax=True,
    figsize=figsize,
...
t_interval, _, _ = arena.ocean_field.hindcast_data_source.convert_to_x_y_time_bounds(
    x_0=x_0.to_spatio_temporal_point(), x_T=x_T, deg_around_x0_xT_box=1, temp_horizon_in_s=3600*24*5
)
...
def plan_traj_from_x_t_and_plot(arena_traj, num_traj_disc, dt_in_sec, ax, rel_time, **kwargs):
    ...
    ax.plot(planner.planned_trajs[-1]['traj'][0, :],
    ...
    ax.plot(x_traj[0, :], x_traj[1, :], linewidth=4, color='black', linestyle='--',
            label="plan at x_t",)
...
while problem_status == 0:
    ...
    if current_fmrc != new_fmrc:
        ...
        planner.plot_reachability_animation(
            time_to_reach=True,granularity_in_h=5,
            temporal_resolution=3600, fps=15,
            with_opt_ctrl=False,
            figsize=figsize,
    ...
    if check_if_new_fmrc(planner, observation) or problem_status != 0:
        ...
        planner.plot_reachability_animation(
            time_to_reach=True, granularity_in_h=5, with_opt_ctrl=False,
            temporal_resolution=1800, fps=15,
