...
def get_copernicus_hindcast(lon_range: List[float], lat_range: List[float], start: datetime.datetime,
    ...
    for day in range(days):
        ...
        ocean_field = OceanCurrentField(hindcast_source_dict=hindcast_dict, casadi_cache_dict=casadi_cache_dict)
        print("Made connection to Copernicus!")
        # get xarray
        ds_hindcast = ocean_field.hindcast_data_source.DataArray
        # slice for specific range
        hindcast = ds_hindcast.sel(time=slice(start+datetime.timedelta(days=day), start+datetime.timedelta(days=1+day)),
                                   lon=slice(*lon_range),
                                   lat=slice(*lat_range))
