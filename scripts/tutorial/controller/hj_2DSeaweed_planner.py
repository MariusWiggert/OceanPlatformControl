...
x_0 = PlatformState(
    ...
    lat=units.Distance(deg=1.5),
    date_time=datetime.datetime(2022, 11, 24, 18, 0, tzinfo=datetime.timezone.utc),
)
x_T = SpatialPoint(lon=units.Distance(deg=1), lat=units.Distance(deg=1.5))
...
t_interval, lat_bnds, lon_bnds = arena.ocean_field.hindcast_data_source.convert_to_x_y_time_bounds(
    x_0=x_0.to_spatio_temporal_point(), x_T=x_T, deg_around_x0_xT_box=2, temp_horizon_in_s=3600
)
...
#%% get state at max value
max = jnp.max(planner.all_values[-1])
state = jnp.where(planner.all_values[-1] == max)
...
for i in tqdm(range(int(200))):  # 3 days
