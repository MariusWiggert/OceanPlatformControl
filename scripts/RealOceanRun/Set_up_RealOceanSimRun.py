...
# add exp
exp = c3.Experiment.createNew(name=experimentName, description='description of experiment', arenaConfig=arena_config,
                        objectiveConfig=objectiveConfig, timeout_in_sec=3600 * 24 * 3)
...
ctrl_config = {
    ...
    "deg_around_xt_xT_box": 0.5,  # area over which to run HJ_reachability
...
missionConfig = {
    "target_radius": 0.05,  # in degrees
    "x_0": [{"date_time": "2023-04-26T15:00:00+00:00", "lat": 37.738160, "lon": -122.545469}],  # the start position and time
    "x_T": {"lat": 37.482812, "lon": -122.531450}}  # the target position
...
real_oc_run = exp.createRealOceanRun(mission_id=mission_id, controller_id=controller_id, observer_id=observer_id)
...
real_oc_run = c3.RealOceanSimRun.get('Ocean_Beach_to_HFB_NOAA_April_Test_api_test_mission_HJ_controller_onFMRC_T_horizon_24h')
start = time.time()
thrust_vec = real_oc_run.getThrustVector(lat=37.738160,
                            lon=-122.545469,
