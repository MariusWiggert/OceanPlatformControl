...
def get_value_function_grid(
    ...
    val_at_t = (
        (val_at_t - val_at_t.min()) * (planner.current_data_t_T - planner.current_data_t_0) / 3600
    )
    ...
    if plot_title is not None:
        ...
        print(
            f"Reach Times: [{planner.reach_times[0]}, {planner.reach_times[1]}, ..., {planner.reach_times[-1]}]"
        )
        ...
        CS = plt.contour(
            planner.grid.states[..., 0],
            planner.grid.states[..., 1],
            val_at_t,
        ...
        plt.scatter(
            planner.grid.states[..., 0].flatten(), planner.grid.states[..., 1].flatten(), s=0.05
        )
    ...
    for t in tqdm(range(steps), disable=verbose < 1):
        ...
        if t >= TRUE_CURRENT_LENGTH:
            ...
            val_copernicus = get_value_function_grid(
                ...
                point,
                TTR_MAP_OUT_WIDTH,
                TTR_MAP_OUT_WIDTH_DEG,
                f"Copernicus ({mission_folder}): Iteration {t})" if plot else None,
            )
            true_currents = np.array(
                [
                    np.array([t["lon"], t["lat"], t["u_true"], t["v_true"]])
                    for t in trajectory[-TRUE_CURRENT_LENGTH:]
                ]
            )
...
if __name__ == "__main__":
    ...
    problem = NavigationProblem(
        ...
        end_region=SpatialPoint(
            lon=units.Distance(deg=-83.17890714569597), lat=units.Distance(deg=18.946404547127734)
        ),
        target_radius=0.1,
        timeout=100 * 3600,
    )
