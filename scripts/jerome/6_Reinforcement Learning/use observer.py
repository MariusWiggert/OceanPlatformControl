...
arena = ArenaFactory.create(
    scenario_name="gulf_of_mexico_Copernicus_forecast_HYCOM_hindcast", problem=problem, verbose=10
)
...
for i in range(1):
    ...
    data = observer.get_data_over_area(
        x_interval=[
            arena_observation.platform_state.lon.deg - WIDTH_DEG / 2,
            arena_observation.platform_state.lon.deg + WIDTH_DEG / 2,
        ],
        y_interval=[
            arena_observation.platform_state.lat.deg - WIDTH_DEG / 2,
            arena_observation.platform_state.lat.deg + WIDTH_DEG / 2,
        ],
        t_interval=[
            arena_observation.platform_state.date_time,
            arena_observation.platform_state.date_time,
        ],
