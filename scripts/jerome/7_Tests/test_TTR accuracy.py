...
@ray.remote
def run_environment(worker_index, higher_resolution) -> OceanEnv:
    env = OceanEnv(
        config={
...
# envs = ray.get([run_environment.remote(i) for i in range(10)])
# fig = plt.figure()
...
envs = ray.get([run_environment.remote(worker_index=1, higher_resolution=False), run_environment.remote(worker_index=1, higher_resolution=True)])
