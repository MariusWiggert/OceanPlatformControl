...
for i in range(1, ITERATIONS + 1):
    ...
    print(' ')
    print(' ')
    print(f'--------- Iteration {i} (total samples {result["info"]["num_steps_trained"]}) ---------')
    ...
    print(f'Min:  {min(result["hist_stats"]["episode_lengths"][-result["episodes_this_iter"]:]):.2f}')
    print(f'Max:  {max(result["hist_stats"]["episode_lengths"][-result["episodes_this_iter"]:]):.2f}')
    print(f'Number of Episodes:   {result["episodes_this_iter"]}')
    print(f'Total Episode Steps:  {result["timesteps_this_iter"]}')
    print(f'Samples for Training: {result["info"]["num_steps_trained_this_iter"]}')
    print(' ')
    print('-- Timing --')
...
# with open(model_path+'/results.jsonn', "w") as outfile:
#     json.dump(results, outfile)
