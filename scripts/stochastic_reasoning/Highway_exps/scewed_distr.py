...
n_mc = 2
u_highway_samples = np.array([-4.5/5, 0.5/5]).reshape((n_mc, 1))
weights_initial = np.array([0.2, 0.8])
...
value_estimator.plot_ttr_snapshot(stoch_params_idx=1, time_idx=-1, granularity=2, time_to_reach=True)
#%%
belief_state_id=40
belief = pomdop_planner.mcts_planner.tree.belief_id_to_belief[belief_state_id]
print(belief.states)
value_estimator(pomdop_planner.mcts_planner.tree.belief_id_to_belief[belief_state_id])
# F_max = 0.4

#%% visualize the combined value function.
val_1 = value_estimator.all_val_funcs[0, -1, ...]
val_2 = value_estimator.all_val_funcs[1, -1, ...]
# combine them weighted with weights_initial
val_at_t = weights_initial[0] * val_1 + weights_initial[1] * val_2
# plot it
...
# get_initial_value
initial_values = self.all_val_funcs[0, 0, ...]
