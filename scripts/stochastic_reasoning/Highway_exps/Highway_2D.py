...
# TODO: can play around with depth of the tree. Here multi-step depth can make sense!
# % Problem Setup
# Start - Goal Settings
init_state = [5., 2.0, 0]
target_state = [5., 8.0]
...
y_highway_center = 5
y_highway_width = 1
true_params = [u_highway_true, y_highway_width]
y_highway = [y_highway_center - y_highway_width, y_highway_center + y_highway_width]
F_max = 0.4
...
# n_mc = 9
# sample gaussian mean u_highway_true and std 0.5
# u_highway_samples = np.random.normal(u_highway_true, scale=0.5, size=n_mc).reshape((n_mc, 1))
u_highway_samples = [-0.5, 0, 0.5]
# sample width of highway distribution
# highway_width_samples = np.random.normal(y_highway_width, scale=0.5, size=n_mc).reshape((n_mc, 1))
highway_width_samples = [0.5, 1, 1.5]
# now create 9 points which are the coordinates of the meshgrid of these params
n_mc = len(u_highway_samples) * len(highway_width_samples)
params_array = []
for i in range(len(u_highway_samples)):
    for j in range(len(highway_width_samples)):
        params_array.append([u_highway_samples[i], highway_width_samples[j]])
# make it a numpy array
highway_params = np.array(params_array)
...
# 2D Highway version from Manan
# essentially: u = A if y in [0.5-w, 0.5+w] else 0 (so centered around 0) and if w=0.5 then u=A if y in [0, 1]
# def anlt_hway_vel(t, x, y, A, w):
#     return jnp.array([A*jnp.logical_and(y-0.5<w,0.5-y<w), jnp.zeros_like(y)])
# Now a function that is centered around 5
def highway_vel_2D(t, x, y, A, w, y_highway_center):
    return jnp.array([A * jnp.logical_and(y_highway_center + w <= y, y <= y_highway_center - w), jnp.zeros_like(y)])
highway_vel_fixed = partial(highway_vel_2D, y_highway_center = y_highway_center)
...
from ocean_navigation_simulator.controllers.pomdp_planners.utilities.analytical_currents import \
    highway_current_analytical2D

particle_vel_func = partial(highway_current_analytical2D,
                            y_highway_center=y_highway_center)
...
mcts_dict = {
    ...
    'no_position_uncertainty': True,
    'resample': False,
    'reward_function': Rewards.TimeRewardFunction(target_state, target_radius),
    # Rewards.RewardFunction(target_state, target_radius),
    'mcts_settings': {
        # 100 is very low, the higher the more flushed out the tree
        "num_mcts_simulate": 100,
        # number of simulate calls to MCTS (either explores children or new node) from root node.
...
pomdop_planner = PomdpPlanner(
    ...
    mcts_dict=mcts_dict,
    initial_weights=weights_initial
)
...
#%% Test to see if data assimilation works
#%% start in the middle, otherwise no signal in the observation
init_filter_test = [5, 3.5, 0]
...
visualize.plot_particles_in_2D(
    particle_belief=pomdop_planner_for_filter_tests.particle_observer.particle_belief_state,
    x_axis_idx=3, y_axis_idx=4, x_axis_label="u_highway", y_axis_label="highway_width", true_state=true_params)
...
visualize.plot_particles_in_2D(
    particle_belief=pomdop_planner_for_filter_tests.particle_observer.particle_belief_state,
    x_axis_idx=3, y_axis_idx=4, x_axis_label="u_highway", y_axis_label="highway_width",
