function hamValue = time_Ham(t, data, deriv, schemeData)
...
%% correct the derivative inside of the area (should be 0)
% But it's not because we use a 4th order schema so it checks outside...
deriv{1, 1}(find(abs(schemeData.grid.xs{1, 1}) < 0.4)) = 0;
...
%% add the Lagrangian term
hamValue = hamValue + (abs(schemeData.grid.xs{1, 1}) >= 0.4);
