...
t_upper = min(x_t[3] + sim.sim_settings['hours_to_sim_timescale'] * 24 * sim.sim_settings["t_horizon_sim"],
...
lon_interval = [x_t[0] - sim.sim_settings["deg_around_x_t"], x_t[0] + sim.sim_settings["deg_around_x_t"]]
lat_interval = [x_t[1] - sim.sim_settings["deg_around_x_t"], x_t[1] + sim.sim_settings["deg_around_x_t"]]
...
def lin_interpo_3D_fields(state, time, field_matrix, grid_0, grid_1, grid_2):
    ...
    idx_point = jnp.array([jnp.interp(state[0], grid_0, np.arange(len(grid_0))),
                           jnp.interp(state[1], grid_1, np.arange(len(grid_1))),
                           jnp.interp(time, grid_2, np.arange(len(grid_2)))]).reshape(-1, 1)
