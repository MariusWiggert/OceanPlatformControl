...
T_init, u_init, x_init, dt = non_lin_opt_utils.solve_time_opt_wo_currents(
    x_0, x_T, N, conv_m_to_deg, u_max
)
...
F = ca.Function(
    ...
    [
        ca.vertcat(
            u[0] / conv_m_to_deg + u_curr_func(ca.vertcat(x[0], x[1])),
...
# state constraints
opti.subject_to(opti.bounded(-98.0, x[0, :], -96.0))
opti.subject_to(opti.bounded(21.0, x[1, :], 23.0))
...
gif_utils.gif_straight_line("trial_3_straight", x_0, x_T, T, dt, fieldset, u_max=1.0)
#%%
# u = np.zeros(u.shape)
#%%
...
#%%
u_curr_func, v_curr_func = non_lin_opt_utils.get_interpolation_func(
    fieldset, conv_m_to_deg, type="bspline"
)
...
f_rk_bspline = ca.Function(
    "f_rk_bspline",
    [x_sym, u_sym],
    [
        ca.vertcat(
            u_sym[0] / conv_m_to_deg + u_curr_func(ca.vertcat(x_sym[0], x_sym[1])),
...
F_rk_bspline = ca.Function(
    "F_rk_bspline", [x_sym, u_sym], [res_rk_bspline["xf"]], ["x", "u"], ["x_next"]
)
...
F_ef_bspline = ca.Function(
    ...
    [
        ca.vertcat(
            x_sym[0] + dt * f_ef_bspline(x_sym, u_sym)[0],
...
# NOTE: have to implement Euler myself, only RK in the package...
u_curr_func, v_curr_func = non_lin_opt_utils.get_interpolation_func(
    fieldset, conv_m_to_deg, type="linear"
)
f_ef_linear = ca.Function(
    "f_ef_linear",
    [x_sym, u_sym],
    [
        ca.vertcat(
...
F_ef_linear = ca.Function(
    ...
    [
        ca.vertcat(
            x_sym[0] + dt * f_ef_linear(x_sym, u_sym)[0],
...
# Simplify API to (x,u)->(x_next)
F_rk_linear = ca.Function(
    "F_rk_linear", [x_sym, u_sym], [res_rk_linear["xf"]], ["x", "u"], ["x_next"]
)
...
for i, x in enumerate(xgrid):
    for j, y in enumerate(ygrid):
        ...
        parcels_interpolate[-j, i] = fieldset.U.spatial_interpolation(ti=0, z=0, y=y, x=x, time=0.0)
#%%
plt.matshow(u_func)
plt.show()
...
pset_sample.execute(
    SampleP, runtime=timedelta(seconds=1), dt=timedelta(seconds=dt)  # the total length of the run
)
pset_sample
#%%
u_0 = fieldset.U.spatial_interpolation(ti=0, z=0, y=x_0[1], x=x_0[0], time=0.0)
v_0 = fieldset.V.spatial_interpolation(ti=0, z=0, y=x_0[1], x=x_0[0], time=0.0)
