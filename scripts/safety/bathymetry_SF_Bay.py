...
# Resolution gebco = 1/240 th degree = 1/4 min
# To get to e.g. 1/12 degree resolution, we need to combine 1/12 * 240 = 20 points in one
lat_coarsening = int(np.rint(res * 240))
lon_coarsening = int(np.rint(res * 240))
# Suppress dask warning about chunking size
print("Starting to coarsen the dataset. This may take a minute.")
with dask.config.set(**{"array.slicing.split_large_chunks": False}):
    if op == "mean":
...
# remove further kinks
coarsened["elevation"].loc[dict(lat=slice(37.89, 38), lon=slice(-122.7,-122.65))] = 10
coarsened["elevation"].loc[dict(lat=slice(37.82, 38), lon=slice(-122.53,-122.4))] = 10
coarsened["elevation"].loc[dict(lat=slice(37.25, 37.275), lon=slice(-122.42,-122.2))] = 10
...
# 0 land, 1 water
xarray_land = xr.where(coarsened["elevation"] > 0, 0, 10000)
# Convert to numpy
data = xarray_land.data
lat = xarray_land.coords["lat"].values
lon = xarray_land.coords["lon"].values
...
min_d_map = bfs_min_distance(data, lat_rad, lon_rad)
ds_distance = convert_np_to_xr(min_d_map, lat, lon)
#%%
